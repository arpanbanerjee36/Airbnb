...
document.querySelectorAll(".nav-dropdown >li >button")[0].addEventListener("click", () => {
    ...
    document.getElementById("loginSignupForm").addEventListener('submit', (e) => {
        ...
        fetchResult.then(res => res.json()).then((response) => {
            if (response.email === undefined || response.phone === undefined) {
                document.querySelector("#userLogin .header .headText").remove();
                document.querySelector("#userLogin .header button").innerHTML = `
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-caret-left-fill" viewBox="0 0 16 16">
                <path d="m3.86 8.753 5.482 4.796c.646.566 1.658.106 1.658-.753V3.204a1 1 0 0 0-1.659-.753l-5.48 4.796a1 1 0 0 0 0 1.506z"/>
                </svg>`;
                document.querySelector("#userLogin .header button").classList = "btn back-btn";
                ...
                //Handling back operation of the login/signup page
                document.querySelector("#userLogin .header .back-btn").addEventListener('click', (e) => {
                    e.preventDefault();
                    document.querySelector("#userLogin .header .headText").remove();
                    document.querySelector("#userLogin .header .back-btn").innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-x"
                        viewBox="0 0 16 16">
                        <path
